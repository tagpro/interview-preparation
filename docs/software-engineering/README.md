...
Eight pages on backend engineering, written to be read in order or dipped into.
...
on the AWS and algorithms pages, and the eleven interactive figures on the
algorithms page. No library is loaded at runtime, on any page. The site loads nothing over the network -- the typefaces
