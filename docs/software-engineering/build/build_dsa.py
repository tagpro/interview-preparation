# -*- coding: utf-8 -*-
"""Assemble dsa.html from its fragments, then splice in the chrome the shared
template does not know about: the three-way Go/Python/Java switch, the
interactive figures, and the series rail.
...
PAGE, TITLE = 'dsa.html', 'Data Structures, In Motion'
...
MARK = ('<span class="m-full">Data Structures &amp; Algorithms</span>'
        '<span class="m-short">DSA</span>')
FRAGMENTS = ['dsa_a.html', 'dsa_b.html', 'dsa_c.html', 'dsa_d.html',
             'dsa_e.html', 'dsa_f.html', 'dsa_g.html', 'dsa_h.html']
...
hero = read('dsa_hero.html')
parts = "\n\n".join(read(f) for f in FRAGMENTS)
foot = read('dsa_foot.html')
n = tpl.build(PAGE, TITLE, MARK, hero, parts + "\n" + foot)

s = open(PAGE, encoding='utf-8').read()

# per-topic kickers, numbered in document order
kick = list(re.finditer(r'<span class="kicker">DSA &middot; (\w+)</span>', s))
for i, m in enumerate(reversed(kick)):
    idx = len(kick) - 1 - i
    s = s[:m.start()] + '<span class="kicker">DSA &middot; %02d</span>' % (idx + 1) + s[m.end():]
...
s = s.replace('<div class="topbar">',
              dsa_ui.CSS.strip() + '\n' + dsa_ui.EARLY_JS.strip() + '\n\n<div class="topbar">', 1)
assert s.count('<button class="toc-toggle"') == 1
s = s.replace('    <button class="toc-toggle"', dsa_ui.BAR + '    <button class="toc-toggle"', 1)
...
for m in re.finditer(r'<section class="topic" id="([^"]+)">', s):
    ...
    body = s[m.end():end]
    missing = [k for k in ('go', 'py', 'java') if 'data-lang="%s"' % k not in body]
    if missing:
...
registered = set(re.findall(r'\breg\("([^"]+)"', dsa_demos.JS))
