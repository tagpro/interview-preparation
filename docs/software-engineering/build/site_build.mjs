...
const PAGES = [
  ...
  { src: 'interview-map.html',        out: 'interview.html',    icon: '🗺️',
    uuid: 'b9b3c754-363e-4a1f-9163-32dda13ac63c' },
];
...
// The way back to the site's front page. A site feature, not a page feature:
// the artifacts are published from these same files and have no front page to
// return to, so the link is added here rather than baked into the sources.
//
// It is a plain anchor, not a scripted control, so it works with JavaScript off
// and the browser can tell the reader where it goes. Sticky to the left edge for
// the same reason Contents is sticky to the right: the bar scrolls sideways when
// it is over budget on a phone, and the two things a reader needs to leave the
// page with should never be the ones that scroll away.
const HOME_CSS = `<style>
.homelink{display:inline-flex;align-items:center;justify-content:center;
  width:31px;height:26px;flex:0 0 auto;border:1px solid var(--line);
  color:var(--ink-faint);text-decoration:none;line-height:0;
  position:sticky;left:0;background:color-mix(in srgb,var(--ground) 88%, var(--surface))}
.homelink:hover{color:var(--ink);border-color:var(--accent)}
.homelink:focus-visible{outline:2px solid var(--accent);outline-offset:-3px}
.homelink svg{width:14px;height:14px;display:block;fill:none;stroke:currentColor;
  stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}
@media print{.homelink{display:none!important}}
</style>`;

// Drawn on the same 24-unit grid as the theme icons so the bar reads as one set,
// and as an outline rather than a glyph because the site self-hosts only the
// latin and greek subsets of its faces -- a house character would fall back to
// whatever the system had, or to a coloured emoji in a monochrome bar.
const HOME_LINK = ROOT_ =>
  `<a class="homelink" href="${ROOT_}/index.html" title="Back to the notes" ` +
  `aria-label="Back to the notes"><svg viewBox="0 0 24 24" aria-hidden="true">` +
  `<path d="M3.5 11 12 4l8.5 7M5.8 9.3V20h12.4V9.3M10 20v-5.4h4V20"/></svg></a>`;
...
for (const page of PAGES) {
  ...
  // Into the bar, ahead of the page's own name.
  const bar = '<span class="mark">';
  if (body.split(bar).length !== 2) {
    console.error(`${page.src}: expected exactly one topbar mark to place the home link before`);
    process.exit(1);
  }
  body = body.replace(bar, HOME_LINK(ROOT) + '\n    ' + bar);
